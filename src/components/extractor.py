...
class AnswerExtractor:
    ...
    def extract(model_response: str, valid_labels: List[str]) -> Optional[str]:
        ...
        # Hỗ trợ markdown formatting: **A**, *A*, __A__
        pattern1 = rf"[Đđ]áp\s*án\s*(?:là|:|chính\s*xác\s*là)?[\s:]*[*_]*({labels_pattern})[*_]*(?:\.|,|\s|$|\*)"
        ...
        pattern2 = rf"(?:[Kk]ết\s*luận|KẾT\s*LUẬN).*?(?:là|:)\s*[*_]*({labels_pattern})[*_]*(?:\.|,|\s|$|\*)"
        ...
        # Hỗ trợ markdown formatting
        pattern3 = (
            rf"[Aa]nswer\s*(?:is|:)?\s*[*_]*({labels_pattern})[*_]*(?:\.|,|\s|$|\*)"
        )
        match = re.search(pattern3, model_response, re.IGNORECASE)
        if match:
            return match.group(1).upper()

        # Pattern 4: "[A]" hoặc "(A)" hoặc "**A**" - trong ngoặc hoặc markdown
        pattern4 = rf"[\[\(*_]+({labels_pattern})[\]\)*_]+"
        matches = re.findall(pattern4, model_response, re.IGNORECASE)
        if matches:
            return matches[-1].upper()

        # Pattern 5: Standalone letter at start of line
        pattern5 = rf"^[*_]*({labels_pattern})[*_]*(?:\.|,|\s|$|\*)"
        match = re.search(
            pattern5, model_response.strip(), re.IGNORECASE | re.MULTILINE
        )
        if match:
            return match.group(1).upper()
        ...
        # Tìm pattern "là X" hoặc ": X" trong tail, hỗ trợ markdown
        pattern6 = rf"(?:là|:)\s*[*_]*({labels_pattern})[*_]*(?:\.|,|\s|$|\*)"
        matches = re.findall(pattern6, tail, re.IGNORECASE)
        if matches:
            return matches[-1].upper()

        # Pattern 7: Tìm chữ cái đứng độc lập trong tail
        pattern7 = rf"\b[*_]*({labels_pattern})[*_]*(?:\.|,|\s|$|\*)"
