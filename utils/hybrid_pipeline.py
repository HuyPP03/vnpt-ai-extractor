"""
Hybrid Pipeline - Tối ưu hóa lựa chọn model dựa trên loại câu hỏi
"""
...
# Try to import improved prompts, fallback to original
try:
    from utils.improved_prompt_builder import PromptSelector, ImprovedPromptBuilder
...
class ConfidenceScorer:
    ...
    @staticmethod
    def calculate_confidence(model_response: str, 
        ...
        Args:
            model_response: Response từ model
            extracted_answer: Đáp án đã trích xuất
            valid_labels: Danh sách label hợp lệ
...
class QuestionDifficulty:
    ...
    def classify_difficulty(cls, item: Dict[str, Any]) -> str:
        """
        Phân loại độ khó dựa trên metadata
        
        Args:
            item: Dictionary chứa câu hỏi và metadata
...
class HybridModelSelector:
    """Lựa chọn model tối ưu dựa trên loại câu hỏi"""
    
    @staticmethod
    def select_model(question_type: str, 
                    difficulty: str,
        ...
        Args:
            question_type: Loại câu hỏi (MATH, CONTEXT, KNOWLEDGE)
            difficulty: Độ khó (precision-critical, compulsory, normal)
            context_length: Độ dài context (sau filtering)
            strategy: Chiến lược (cost-optimized, quality-optimized, hybrid)
            
        Returns:
            Model name: "small", "large", hoặc "small_with_fallback"
        """
    ...
    @staticmethod
    def _cost_optimized(question_type: str, difficulty: str, 
    ...
    @staticmethod
    def _quality_optimized(question_type: str, difficulty: str,
                          context_length: int) -> str:
        """Chiến lược tối ưu chất lượng"""
        return "large"  # Tất cả dùng large
    
    @staticmethod
    def _hybrid_strategy(question_type: str, difficulty: str,
                        context_length: int) -> str:
        """Chiến lược hybrid cân bằng"""
        # MATH: Luôn large
        if question_type == "MATH":
            return "large"
...
class OptimizedHybridPipeline:
    ...
    def __init__(self, strategy: str = "hybrid", use_improved_prompts: bool = True,
                 large_model_name: str = "large", small_model_name: str = "small"):
        """
        Args:
    ...
    def process_single(self, item: Dict[str, Any], 
                      verbose: bool = False) -> Dict[str, Any]:
        """
        Xử lý một câu hỏi với hybrid strategy
        
        Args:
            item: Dictionary chứa question, choices, answer (optional)
            verbose: In chi tiết
        ...
        ground_truth = item.get("answer", "").strip().upper() if "answer" in item else None
        ...
        if question_type == "CONTEXT":
            ...
            if context_length > 1000:
                ...
                if verbose:
                    print(f"Context filtered: {metadata['original_length']} → {metadata['filtered_length']} chars")
        ...
        if self.use_improved_prompts:
            ...
            prompt = self.prompt_selector.select_prompt(
                ...
                model_type=selected_model if selected_model != "small_with_fallback" else "small"
            )
        else:
    ...
    def _get_model_response(self, model_type: str, prompt: str,
                           question_type: str, choices: List[str],
                           verbose: bool = False) -> Dict[str, Any]:
        """
        Gọi model và trích xuất đáp án
        """
        # Get model
        ...
        try:
            ...
            # Validate
            is_valid = self.formatter.validate_answer(predicted, choices) if predicted else False
    ...
    def _process_with_fallback(self, prompt: str, question_type: str,
                              choices: List[str], verbose: bool = False) -> Dict[str, Any]:
        """
        Thử small trước, nếu thất bại hoặc confidence thấp thì retry với large
        """
        if verbose:
...
def create_pipeline(strategy: str = "hybrid") -> OptimizedHybridPipeline:
    ...
    Args:
        strategy: "cost-optimized", "quality-optimized", hoặc "hybrid"
        
    Returns:
        OptimizedHybridPipeline instance
    """
...
if __name__ == "__main__":
    # Example usage
    pipeline = create_pipeline(strategy="hybrid")
    
    # Test với một câu hỏi
    test_item = {
        "qid": "test_001",
        "question": "Việc đưa ra các quy định về thuế, pháp luật đã tác động đến lĩnh vực:",
        "choices": ["Môi trường", "Kinh tế", "Văn hóa", "Quốc phòng an ninh"],
        "answer": "B"
    }
    
    result = pipeline.process_single(test_item, verbose=True)
    print("\nResult:", result)
