...
class SemanticContextFilter:
    """
    Filter context sử dụng embedding-based semantic search
    Tổng quát và mạnh mẽ hơn keyword matching
    """
    
    def __init__(self):
        """
        Args:
            embedding_model: Model để tạo embeddings (VNPT/Sentence-Transformers/OpenAI)
        """
    ...
    def _simple_embedding(self, text: str, dim: int = 384) -> np.ndarray:
        """
        Fallback embedding đơn giản dựa trên word frequency
        Được sử dụng khi không có embedding model
        """
        # Normalize text
        text = text.lower()
    ...
    def split_into_chunks(self, text: str, chunk_size: int = 200, 
        ...
        # Try to split by sections first (paragraphs/headers)
        ...
        if sections:
            ...
            for section in sections:
                ...
                if len(section_text) > chunk_size:
                    sub_chunks = self._split_long_text(section_text, chunk_size, overlap)
            ...
            chunks = [{'text': c, 'section_title': '', 'is_partial': True} for c in chunks]
    ...
    def retrieve_relevant_chunks(self, context: str, question: str,
                                top_k: int = 5, 
                                min_similarity: float = 0.3) -> List[Tuple[str, float]]:
        """
        Retrieve top-k chunks liên quan nhất đến question
        ...
        for chunk in chunks:
            ...
            if chunk['section_title']:
                ...
                title_similarity = self.cosine_similarity(question_embedding, title_embedding)
        ...
        relevant_chunks = [
            (text, score) for text, score in chunk_scores[:top_k]
            if score >= min_similarity
        ]
        
        return relevant_chunks
    
    def filter_context(self, context: str, question: str,
                      max_chunks: int = 3,
                      max_chars: int = 1000) -> Tuple[str, Dict[str, Any]]:
        """
        Filter context và trả về phần liên quan nhất
        ...
        metadata = {
            ...
            'avg_similarity': sum(s for _, s in relevant_chunks[:chunks_used]) / chunks_used if chunks_used > 0 else 0,
