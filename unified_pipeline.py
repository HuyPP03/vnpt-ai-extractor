"""
Unified Pipeline - Hỗ trợ cả Baseline và Hybrid strategies
Hỗ trợ VNPT AI và OpenAI models
"""
...
class UnifiedPipeline:
    """
    Unified pipeline hỗ trợ nhiều strategies và model providers
    """
    ...
    def __init__(
        ...
        """
        Args:
            strategy: baseline, hybrid, cost-optimized, quality-optimized
            model_provider: vnpt hoặc openai
            large_model: Tên large model (large/gpt-4o-mini)
            small_model: Tên small model (small/gpt-3.5-turbo)
            use_improved_prompts: Sử dụng improved prompts
        """
    ...
    def _create_pipeline(self):
        """Tạo pipeline dựa trên strategy"""
        if self.strategy == "baseline":
            # Baseline: Dùng model cố định (thường là large)
            ...
            # Resolve model names
            large_model_resolved = self._resolve_model_name(self.large_model)
            small_model_resolved = self._resolve_model_name(self.small_model)
            ...
            return OptimizedHybridPipeline(
                ...
                large_model_name=large_model_resolved,
                small_model_name=small_model_resolved
            )
    
    def _resolve_model_name(self, model: str) -> str:
        """Resolve model name dựa trên provider"""
        if self.model_provider == "openai":
            # OpenAI models - return actual model name
            # Map generic names to specific OpenAI models
            if model == "large":
                return "gpt-4o-mini"
            elif model == "small":
                return "gpt-3.5-turbo"
            else:
                # Return as-is for specific OpenAI model names
                return model
        else:
            # VNPT models
            return model
    ...
    def evaluate(
        ...
        Args:
            ...
            save_predictions: Lưu predictions
    ...
    def _print_result(self, result: Dict[str, Any]):
        ...
        if result.get("correct") is not None:
            ...
            print(f"{status} {model_info} Predicted: {result['predicted']}, GT: {result['ground_truth']}")
        else:
            print(f"Predicted: {result['predicted']} [{result.get('model_used', 'unknown')}]")
    ...
    def _create_summary(self, questions: List[Dict], results: List[Dict], 
        ...
        summary = {
            ...
            "avg_time_per_question": f"{total_time/len(questions):.2f}s" if questions else "0s",
            "results": results
        }
        ...
        if has_ground_truth:
            ...
            accuracy = (self.stats["correct"] / total_valid * 100) if total_valid > 0 else 0
    ...
    def _print_summary(self, summary: Dict[str, Any]):
        ...
        if "model_usage" in summary:
            ...
            print(f"\nModel Usage:")
            print(f"  Small: {usage.get('small_used', 0)} ({usage.get('small_percentage', '0%')})")
            print(f"  Large: {usage.get('large_used', 0)} ({usage.get('large_percentage', '0%')})")
            print(f"  Fallback triggered: {usage.get('fallback_triggered', 0)} ({usage.get('fallback_rate', '0%')})")
        ...
        if "accuracy_metrics" in summary:
            ...
            if "accuracy_by_type" in summary:
                ...
                for qtype, stats in summary["accuracy_by_type"].items():
                    print(f"  {qtype}: {stats['accuracy']} ({stats['correct']}/{stats['total']})")
        
        print(f"{'='*70}\n")
    
    def _save_results(self, summary: Dict[str, Any], save_results: bool,
                     save_predictions: bool, output_dir: str):
        """Lưu kết quả"""
...
Examples:
  ...
  python unified_pipeline.py --strategy hybrid --save-predictions
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="End index (default: end of list)"
    )
